// Package bot — pairing-code (TOFU) manager.
//
// PairingManager mints short, time-limited, single-use codes per bot UUID.
// Codes are kept only in memory: a server restart invalidates outstanding
// codes by design. Verification is constant-time and rate-limited; after
// too many wrong attempts the bot is temporarily locked out from further
// pairing attempts.
package bot
...
func NewPairingManager(auditLog *audit.Logger, opts ...PairingManagerOption) *PairingManager {
	p := &PairingManager{
...
func (p *PairingManager) Mint(botUUID string) (string, time.Time) {
	...
	p.auditEvent(audit.LevelInfo, "imbot.pair.code_minted", "", "pairing code minted",
...
func (p *PairingManager) Verify(botUUID, candidate string) error {
	...
	if att, ok := p.attempts[botUUID]; ok && att != nil && now.Before(att.lockedUntil) {
		p.mu.Unlock()
		p.auditEvent(audit.LevelWarn, "imbot.pair.fail", "", "verify rejected by lockout",
			map[string]interface{}{
				"bot_uuid":     botUUID,
				"reason":       "locked",
...
func (p *PairingManager) recordFailure(botUUID, reason string) {
	...
	p.auditEvent(audit.LevelWarn, "imbot.pair.fail", "", "verify failed",
		map[string]interface{}{
			"bot_uuid": botUUID,
			"reason":   reason,
			"fails":    fails,
	...
	if locked {
		p.auditEvent(audit.LevelWarn, "imbot.pair.locked", "", "pairing locked after repeated failures",
			map[string]interface{}{
				"bot_uuid":     botUUID,
...
func (p *PairingManager) auditEvent(level audit.Level, action, userID, message string, details map[string]interface{}) {
	if p.audit == nil {
		return
	}
	switch level {
	...
	case audit.LevelError:
		p.audit.Error(action, userID, "", message, false, details)
	default:
