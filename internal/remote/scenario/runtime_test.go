...
func (c *recordingChannel) Capabilities() channel.Capabilities { return channel.Capabilities{} }
...
func (c *recordingChannel) Prompt(ctx context.Context, t channel.Target, ix interaction.Interaction) (interaction.Reply, error) {
...
func TestRuntimeResolveNoBinding(t *testing.T) {
	rt := NewDefaultRuntime(channel.NewRegistry(), binding.NewResolver(&fakeStore{}), nil)
...
func TestRuntimeResolveBoundButNotRunning(t *testing.T) {
	...
	rt := NewDefaultRuntime(channel.NewRegistry(), binding.NewResolver(store), nil)
